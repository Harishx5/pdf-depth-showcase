...
const Education: React.FC = () => {
  return (
    <section id="education" className="section-padding max-w-6xl mx-auto">
      <ScrollAnimation>
        <p className="text-primary text-sm tracking-widest uppercase mb-3">Education</p>
          ...
          {education.map((edu, i) => (
            <ScrollAnimation key={i} delay={0.1 + i * 0.12} direction="left">
              <div className="flex items-start gap-6 md:gap-8 relative">
                <div className={`w-12 h-12 md:w-16 md:h-16 rounded-xl flex items-center justify-center shrink-0 z-10 ${
                  edu.current ? 'bg-primary/20 border border-primary/40' : 'bg-secondary border border-border'
                }`}>
                ...
                <div className="glass rounded-2xl p-6 flex-1">
                  <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-1 mb-2">
                    ...
                    {edu.current && (
                      <span className="text-xs px-3 py-1 rounded-full bg-primary/10 text-primary border border-primary/20 w-fit">
                        Pursuing
                      </span>
                    )}
                  </div>
