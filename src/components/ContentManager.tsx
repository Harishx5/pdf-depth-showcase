...
function AIProjectsEditor({ data, setData }: { data: any; setData: (d: any) => void }) {
  ...
  const handleScreenshotUpload = async (i: number, file: File) => {
    ...
    const path = `ai-project-${Date.now()}-${i}.${ext}`;
  ...
  const handleRemoveScreenshot = async (i: number) => {
    ...
    if (url) {
      ...
      if (parts[1]) await supabase.storage.from('project-screenshots').remove([decodeURIComponent(parts[1])]);
    }
  ...
  return (
    <div className="space-y-4">
      ...
      <Button variant="outline" size="sm" onClick={() => setData({ ...data, projects: [...data.projects, { title: '', description: '', tech: [], liveUrl: '', githubUrl: '', screenshot: '' }] })}><Plus className="w-4 h-4 mr-1" /> Add Project</Button>
    </div>
  );
}
